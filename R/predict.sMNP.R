predict.sMNP <- function(object, newdata = NULL, newdraw = NULL, n.draws = 1,
  ...
  coef <- param[,1:n.cov]
  n.mcmc <- nrow(coef)
  cov <- param[,(n.cov+1): (ncol(param)-1)]
  baseSamp <- param[, ncol(param)]
  ...
  if (is.null(newdata)) 
    ...
    call <- object$call
    x <- xmatrix.sMNP(as.formula(call$formula), data = newdata,
                     choiceX = call$choiceX,
                     cXnames = eval(call$cXnames),
                     n.dim = p, lev = object$alt,
...
#    alt <- c(object$base, alt[1:(length(alt)-1)])
  ...
  res <- .C("predict", as.double(x), as.integer(n.obs),
            as.double(coef), as.double(cov), as.integer(baseSamp), as.integer(p),
